...
bool HelloWorldPublisher::init(const std::string& dym_type_name)
{
    std::cout << "====\n Starting Pub with type: " << dym_type_name << std::endl;
    ...
    eprosima::fastrtps::types::DynamicType_ptr dyn_type =
        eprosima::fastrtps::xmlparser::XMLProfileManager::getDynamicTypeByName(dym_type_name)->build();
    ...
    m_Hello->set_string_value("Hello World with " + dym_type_name, 0); // message field
    m_Hello->set_string_value("v0.1.2", 3); // version field
    m_Hello->set_uint32_value(0, 1);  // index filed
    ...
    topic_ = mp_participant->create_topic(
        "DDSDynHelloWorldTopic", dym_type_name, TOPIC_QOS_DEFAULT);
...
/// Note: This will not get called when the there's no matching
...
void HelloWorldPublisher::PubListener::on_offered_deadline_missed(
        DataWriter* writer,
...
{
    /// This line will check if there's matching listener for that topic
