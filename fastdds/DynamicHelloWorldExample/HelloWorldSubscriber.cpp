...
{
    std::cout << "starting dynamic hello world Sub" << std::endl;
        ...
        eprosima::fastrtps::types::DynamicType_ptr dyn_type)
{
    ...
    std::cout << "hash type: " << identifier->equivalence_hash() << std::endl;
    ...
    // Topic Listener, to experiment with "on_inconsistent_topic" callback
    // NOTE(YL): callback is not working currently
    CustomTopicListener custom_topic_listener;

    const std::string type_name = "CustomMsgBar";
    // const std::string type_name = m_type->getName(); /// with this, get dym get type name from pub
    std::cout << "<<< create topic, with type name : " << type_name << std::endl;
    //CREATE THE TOPIC
    Topic* topic = subscriber_->mp_participant->create_topic(
        "DDSDynHelloWorldTopic",
        type_name,
        TOPIC_QOS_DEFAULT,
        &custom_topic_listener
        );
    std::cout << "<< end create topic" << std::endl;
    ...
    DataReader* reader = subscriber_->mp_subscriber->create_datareader(
        ...
        sub_mask);
    std::cout << "data reader created" << std::endl;
